---
title: Complementarity-determining regions
tags:
  - citation-fix
created: 2026-04-10T14:02:57
modified: "2026-04-21T07:03:26"
---

The **complementarity-determining regions** of [[tags/antibodies|antibodies]], abbreviated **CDRs** and sometimes collectively called **hypervariable regions**, are loops that mediate binding to antigens. They are the most difficult part of the protein to predict and get fine-tuned by [[tags/affinity-maturation|affinity maturation]] and [[Somatic hypermutation|somatic hypermutation]].

### General observations
#### Confomational clustering

*Conformational clusters of CDRs were first described by Chothia & Lesk [@chothia1987]*

### CDRH1

* **Camelid [[Nanobodies|nanobodies]] account for most of the diversity in CDRH1 lengths, which in humans and mice are almost all length 7** [@nowak2016].

### CDRH2

* **The residue identity at heavy chain residue 71 (Chothia numbering) influences the conformation of CDRH2** ([[_Articles that need citations|citation needed]]).
* **In Kabat numbering CDRH2 is slightly too short (reason unclear).**
* **[[HDX-MS]] data suggest that CDRH2 has decreased dynamics following [[tags/affinity-maturation|Affinity maturation]]** ([[_Articles that need citations|citation needed]]).

### CDRH3
##### Conformation and dynamics

*See [[Kinked CDRH3 loops]]*

### CDRL1

* **Longer CDRL1 loops correlate with restricted CDRH3 dynamics** [@guloglu2023].

### CDRL2

* **[[HDX-MS]] data suggest that CDRH2 has decreased dynamics following [[tags/affinity-maturation|affinity maturation]]** ([[_Articles that need citations|citation needed]]).
* **In Kabat numbering this is slightly too short** (reason unclear; [[_Articles that need citations|citation needed]]).

### CDRL3

* **Shorter CDRL3 loops correlate with restricted CDRH3 dynamics** [@guloglu2023].
* **More variable than CDRL1 or CDRL2 due to [[Somatic hypermutation|VJ recombination]]** [@nowak2016].

### CDR4

*Also known as the DE loop*
* Sequence variation in the DE loop exceeds that of the framework
* Length variation found in [[Broadly neutralizing antibodies|broadly neutralizing antibodies]]
* **Also known as the DE motif, since it connects the D and E strands of [[tags/antibodies|Antibodies]]; the name CDR4 is less common.**
* **PyIgClassify2 found four distinct clusters for length-6 light chain.** Two of these with exclusively [[tags/light-chains#Kappa subtype|kappa subtype]], one exclusively with [[tags/light-chains#Lambda subtype|lambda subtype]], and one mixed with both; almost all heavy chain structures have length-8 CDR4 and co-cluster [@kelow2020; @kelow2022].
* **Also used in [[Nanobodies]] and can be engineered to improve binding** [@ketaren2023]
* **Fernandez-Quintero et al. [@fernandezquintero2020] and Kelow et al. [@kelow2020] found that this can modulate the conformation of other CDRs, particularly CDRL1**
![[a-ferviously-noted.jpg]]
