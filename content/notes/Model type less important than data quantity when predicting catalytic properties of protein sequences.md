---
tags:
  - protein-design/misc
created: "2024-11-14T03:14:19"
modified: "2026-04-05T23:36:09"
---

#### Summary

**The type of predictive model used is less important than the quantity of data available when [[Fitness prediction|predicting catalytic properties]] of protein sequences** [^muir2024].

#### Figures

![](/assets/Pasted-Graphic-10-1.png)

*Figure from [^muir2024]*

[^muir2024]: Muir et al. (2024) "Evolutionary-Scale Enzymology Enables Biochemical Constant Prediction Across a Multi-Peaked Catalytic Landscape." https://doi.org/10.1101/2024.10.23.619915
