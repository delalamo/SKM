...
#### Details
The perplexity values of null models was calculated below by Ingraham et al 2019[^ingraham2019]:

![](/assets/Table-1-Null-perplexities-for-common-statistical-models-of-proteins.png)
*Table from Ingraham et al 2019[^ingraham2019]*

Hie et al 2022[^hie2022] found that sequence length does not consistently correlate with perplexity values within protein families.

Meier et al 2022[^meier2022] introduced ways of calculating probabilities or "energies" of sequences using masked [[Protein language models|LMs]] (e.g., how favored or disfavored they are given evolution). The authors of [[ESM]]-1v propose four approaches for this:
...
- **Pseudo-likelihood:** Devkota et al 2024[^devkota2024] found that [[ESM]]2 pseudo-log likelihood values scaled linearly with the length of the sequence. They introduced a length-invariant correction ($pLL_{invar}$): $pLL(S)=\sum{\log{p(x_{i}=x_{i}^{mnt}|x_{i-1}^{mnt}) - \log{p(x_{i}=x_{i}^{wt}|x_{i-1}^{wt})}}}$, $pLL_{invar}(S)=\frac{pLL(S)}{|-0.406*len(S)+1.363|}$

![](/assets/Pasted-image-20240820153913.png)
*Figure from Devkota et al 2024[^devkota2024]*
...
Gordon et al 2024[^gordon2024] propose a way to approximate sequence log-likelihood in a single pass using masked LMs:

![](/assets/Pasted-image-20241004100612.png)
![](/assets/Pseudo-Likelihoods.png)
...
[^ingraham2019]: Ingraham et al 2019. https://doi.org/10.5555/3454287.3455704
