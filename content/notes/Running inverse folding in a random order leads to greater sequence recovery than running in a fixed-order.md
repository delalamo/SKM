...
---
#### Summary
**Running [[Inverse folding]] in a random order outperforms fixed order** (Dauparas et al 2022[^dauparas2022]). A related scheme is order-agnostic autoregressive diffusion, used by a sequence diffusion model, in which the identities of individual residues are determined one at a time. The forward noising process masks one residue at a time, in effect. This was shown by Alamdari et al 2023[^alamdari2023] to outperform whole-sequence simultaneous diffusion.  Applying this scheme on diffusion models for [[Multiple sequence alignments]] outperforms [[MSA Transformer]].
