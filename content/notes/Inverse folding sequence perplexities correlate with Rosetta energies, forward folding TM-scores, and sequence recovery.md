---
tags:
  - protein-design/misc
---
#### Summary
 **[[Inverse folding]] perplexities/log-likelihood values correlate with energies calculated with [[Rosetta]], [[TM-score]] of forward-folded decoys using [[OmegaFold]]** (Liu et al 2024c[^liu2024])**, and sequence recovery** (Akpinaroglu et al 2023[^akpinaroglu2023]).

#### Figures
![](/assets/Pasted-image-20240418073054.png)
*Figure 4 from Liu et al 2024c[^liu2024]*

![](/assets/Pasted-image-20240418073310.png)
*Figure 1 from Akpinaroglu et al 2023[^akpinaroglu2023]*

#### See also
* [[Sequence recovery of inverse folding lower for smaller proteins]]

[^liu2024]: Liu et al. (2024) "All-atom protein sequence design based on geometric deep learning." https://doi.org/10.1101/2024.03.18.585651
[^akpinaroglu2023]: Akpinaroglu et al. (2023) "Structure-conditioned masked language models for protein sequence design generalize beyond the native sequence space." https://doi.org/10.1101/2023.12.15.571823
