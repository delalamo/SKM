---
tags: [protein-design/misc]
created: "2024-04-24T16:23:56"
modified: "2026-04-05T23:14:54"
---

---
summary: Inverse folding has lower sequence recovery with smaller proteins
tags:
  - inverse-folding/observations 
---
#### Summary
**[[Inverse folding]] methods have lower sequence recovery on smaller proteins** (Wang et al 2023b[^wang2023]).

#### Figures
![](/assets/sequence_recovery_small_protein.png)
*Figure 2A from Wang et al 2023b[^wang2023]*

[^wang2023]: Wang et al. (2023) "SPDesign: protein sequence designer based on structural sequence profile using ultrafast shape recognition." https://doi.org/10.1101/2023.12.14.571651
