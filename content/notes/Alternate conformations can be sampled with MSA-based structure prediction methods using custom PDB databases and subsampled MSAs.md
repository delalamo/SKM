...
**Alternate conformations can be sampled with MSA-based methods [[AlphaFold|AlphaFold2]] and [[Boltz]] by using either custom templates** (del Alamo et al 2022b[^del2022]) **or custom sequences databases** (Monteiro da Silva et al 2024[^monteiro2024]) **or [[Multiple sequence alignments|MSAs]]** (Wayment-Steele et al 2023[^waymentsteele2023], del Alamo et al 2022b[^del2022]). For the last case, MSAs are modified either by [[Clustering|clustering]] using HDBSCAN or randomly subsampling, respectively. However, [[Protein structure prediction methods are unable to predict the energetics of a conformational landscape unless explicitly trained for that purpose|these ensembles do not correspond to the energetics of those proteins]]. MSA-based tricks were recently shown to work with Boltz ([[U87XyMPrZp|Richman et al 2025]]). These hacks circumvent the broader tendency of structure prediction methods to [[Structure prediction methods undersample the conformational space they find to be high-confidence|undersample conformations they find to be high-confidence]].

#### Details

Alternate conformations sampled via

- **Subsampled MSAs**: Fewer sequences are passed in the MSA (del Alamo et al 2022b[^del2022]). Required for addition of other restraints (e.g., via AlphaLink, Stahl et al 2023[^stahl2023]). Also works for [[Complementarity-determining regions|CDRs]] of [[Nanobodies]] (Riccabona et al 2024[^riccabona2024]).
- **Masked MSA columns**: Entire columns of residues are mutated to either alanine or masked (Stein & Mchaourab 2022[^stein2022], Kalakoti & Wallner 2024[^kalakoti2024])
- **Clustered MSAs**: Wayment-Steele et al 2023[^waymentsteele2023]
- **Template curation**: Can more reliably influence which conformation is sampled, but requires an experimental structure (though Faezov and Dunbrack 2023[^faezov2023] used models for some of their [[Kinases|kinases]]).
- **Dropout**: Shown by Kalakoti & Wallner 2024[^kalakoti2024] to not be very effective, unlike for docking ([[Dropout improves AF2 multimer prediction, including for antibody-antigen complexes|link]])

Vani et al 2023[^vani2023] summarized the challenge of MSA subsampling as follows:
...
*Figure from Riccabona et al 2024[^riccabona2024]*

![](/assets/Pasted-image-20241016074333.png)

*Figure from Schafer & Porter 2024[^schafer2024]*
