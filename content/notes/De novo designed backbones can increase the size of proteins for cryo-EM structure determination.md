---
title: De novo designed backbones can increase the size of proteins for cryo-EM structure determination
tags:
  - design/modular-proteins
  - design/backbones
created: "2024-12-04T02:49:44"
modified: "2026-04-21T07:28:09"
---
#### Summary
 ***De novo* [[notes/protein-backbone-design|designed backbones]] can be used to increase the size of proteins for [[cryo-EM]] structure determination** [@gao2024]. In one case, a domain designed with [[notes/diffusion-models|RF-diffusion]] was inserted into an intracellular loop of a [[G protein-coupled receptors|G-protein coupled receptor]].

#### Details
Its design needed to proceed in a stepwise manner to avoid the diffused domain from re-entering the membrane.

#### Figures
![[Pasted-image-20241004161950.png]]
*Ref [@gao2024]*

#### See also
* [[Antibodies and nanobodies can desymmetrize proteins for cryoEM]]
* [[Membrane proteins can be bulked up for cryo-EM by inserting designed domains, including those with predefined epitopes for antibodies]]
